...
const OUTPUT_PATH = join(ROOT, "supabase", "seed", "curriculum_math.sql");
...
function renderTopicInsert(curriculum: CurriculumCode, topic: TopicRow): string {
  ...
  return [
    ...
    `WHERE c.code = ${sqlLiteral(curriculum)} AND s.code = ${sqlLiteral(MATHEMATICS_CODE)}`,
...
async function exportCurriculumMath(): Promise<string> {
  ...
  const curricula: CurriculumCode[] = ["CBC", "KCSE"];
  ...
  for (const curriculum of curricula) {
    ...
    for (const topic of (topics ?? []) as TopicRow[]) {
      lines.push("", renderTopicInsert(curriculum, topic));
      ...
      for (const subtopic of (subtopics ?? []) as SubtopicRow[]) {
        lines.push("", renderSubtopicInsert(curriculum, topic.code, subtopic));
        ...
        for (const lesson of (lessons ?? []) as LessonRow[]) {
          lines.push(
            "",
            renderLessonInsert(curriculum, topic.code, subtopic.code, lesson),
          );
        }
      }
  ...
  for (const entry of allQuestions) {
    lines.push("", renderQuestionInsert(entry.curriculum, entry.topicCode, entry.question));
  }
...
async function main() {
  loadEnvLocal();
  const sql = await exportCurriculumMath();
  writeFileSync(OUTPUT_PATH, sql, "utf-8");
  console.log(`Exported published Mathematics content to ${OUTPUT_PATH}`);
}
