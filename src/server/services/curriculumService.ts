...
export { isTopicVisibleForGrade, TIER1_SUBJECT_CODES };
export type { Tier1SubjectCode };
...
export async function getSubjectsForStudent(
  ...
  for (const topic of topics ?? []) {
    if (
      !isTopicVisibleForGrade(
        topic.min_grade_sort_order ?? 1,
        studentGradeSortOrder,
      )
    ) {
      continue;
    }
    ...
    topicCountBySubject.set(
      topic.subject_id,
      (topicCountBySubject.get(topic.subject_id) ?? 0) + 1,
    );
  }
