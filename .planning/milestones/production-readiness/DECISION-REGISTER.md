...
| DEC-003 | 2026-06-29 | May beta OAuth bypass the invite gate while email signup cannot? | PR-055, PR-054 | `nexus-map.md` §5.2–5.3; `signupAction`; `/auth/callback`; [ARCHITECT-AMENDMENT.md](./phases/phase-04/ARCHITECT-AMENDMENT.md) §3 | (A) Enforce identical beta policy on OAuth and email; (B) Allow OAuth without invite in beta; (C) Disable OAuth in beta | **SELECTED: (A)** — fail-closed parity; invite carried through OAuth callback and validated server-side in `signInWithGoogleAction` + `/auth/callback` | security constraint + Architect recommendation |
...
| DEC-010 | 2026-06-29 | Daraja callback trust when provider does not sign webhooks? | PR-001, PR-002, PR-003, PR-112, PR-124 | `/api/mpesa/callback`; [ARCHITECT-AMENDMENT.md](./phases/phase-03/ARCHITECT-AMENDMENT.md) §Daraja primary contract; Safaricom Lipa na M-Pesa Online (STK Push `POST /mpesa/stkpush/v1/processrequest`, STK Query `POST /mpesa/stkpushquery/v1/query`); portal: `https://developer.safaricom.co.ke/lipa-na-m-pesa-online/apis/post/stkpush/v1/processrequest` | (A) Opaque per-payment callback secret in URL path + mandatory STK Query reconciliation before activation; (B) IP allowlist only (insufficient alone); (C) Delay activation until manual ops | **SELECTED: (A)** — Daraja does not sign STK callbacks; callbacks are notifications only; `queryStkPush` is independent proof before `verified-paid` | provider contract |
...
| DEC-016 | 2026-06-29 | How to recover the unauthorized Phase 03 migration on the production Supabase project? | PR-001, PR-003, PR-056, PR-096, PR-124 | Production project `frspuvuzvvrebuzzduqg`; remote migration/grant catalog; 43 test users and 49 test payments | (A) lossy rollback; (B) retain and forward-fix; (C) disable payments indefinitely | **SELECTED: (B)** by user — retain migration, apply service-role-only atomic forward repair, remove scoped test fixtures, prohibit production-backed tests | explicit user decision + security constraint |
