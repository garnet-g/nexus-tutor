...
|---|---|---|---|---|---|---|---|---|---|
| PR-001 | §12 P0.1 | P0 | Student billing | `/api/mpesa/callback` accepts unauthenticated POST | Callback requires provider-supported verified channel; anonymous forgery returns 401/403 | 03 | Adversarial test: unsigned POST with forged success → no paid state | PLANNED | `src/app/api/mpesa/callback/route.ts` |
| PR-002 | §12 P0.1 | P0 | Student billing | Checkout request ID returned to student treated as payment proof | Activation requires independent Daraja verification, not checkout ID alone | 03 | Test: initiate STK, forge callback with checkout ID → subscription stays pending | PLANNED | `src/app/api/mpesa/stk-push/route.ts` |
| PR-003 | §12 P0.1; catalog replay | P0 | Student billing | No replay/expiry controls on callback events | Idempotent callback ledger; replay/stale rejected | 03 | Test: duplicate/reordered verified callbacks → single activation | PLANNED | `mpesa_callbacks` migration |
| PR-004 | §12 P0.2 | P0 | Student billing | `isMock=true` path marks payment paid | Production cannot mark paid when mock flag set | 03 | Env matrix: prod + missing Daraja → checkout fails closed | PLANNED | `src/lib/mpesa/mpesaClient.ts` |
| PR-005 | §12 P0.2 | P0 | Ops/deploy | Missing Daraja creds allow paid activation | Startup/deploy validation fails when paid checkout enabled without creds | 02 | `npm run env:check` fails on incomplete M-Pesa in production fixture | PLANNED | Phase 02 env schema |
| PR-006 | §12 P0.3 | P0 | Student Nex | Gemini returns mock text when key missing | Production returns explicit unavailable error, no fake tutor response | 02 | Prod fixture Nex chat → 503/config error, no mock content persisted | PLANNED | `src/lib/nex/` model calls |
| PR-007 | §12 P0.3 | P0 | Student Nex camera | Camera/OCR mocks success without vision creds | Fail closed in production; no fake OCR session | 02 | `/api/nex/camera` prod fixture → error, no persisted fake extraction | PLANNED | `src/app/api/nex/camera/route.ts` |
| PR-008 | §12 P0.3 | P0 | Student Nex voice | Voice mocks transcription/TTS without creds | Fail closed in production | 02 | `/api/nex/voice` prod fixture → error | PLANNED | `src/app/api/nex/voice/route.ts` |
| PR-009 | §12 P0.3 | P0 | Parent/notifications | Celcom mock records delivered SMS | Production never records mock SMS as delivered | 02 | Notification send prod fixture → failed/suppressed, not mock-delivered | PLANNED | Celcom client |
| PR-010 | §12 P0.3 | P0 | Parent/notifications | Resend mock records sent email | Production never records mock email as sent | 02 | Email send prod fixture → fail closed | PLANNED | Resend client |
| PR-011 | §12 P1.1 | P1 | CI/release | `tsc --noEmit` fails on dotAll at test:68,115 | Standalone typecheck passes | 01 | `npm run typecheck` exit 0 | PLANNED | `tests/content/kcseMathSeedContent.test.ts` |
| PR-012 | §11; STATUS | P1 | Content CI | Map claims missing F4 B2 migration; test may desync | Test references `20260625240000_kcse_math_f4_b2.sql` on branch | 01 | Focused content test + migration list pass | PLANNED | `supabase/migrations/20260625240000_kcse_math_f4_b2.sql` |
| PR-013 | §12 P1.2; §6.4 | P1 | Support admin | `/admin/usage-stats` SSR calls service-role snapshot without super-admin guard | Page guard matches API; support cannot read flagged Nex bodies/cost | 04 | Support session GET page → 403; super admin → 200 | PLANNED | `src/app/admin/usage-stats/page.tsx` |
...
| PR-017 | §12 P1.4 | P1 | CI/security | High `undici` advisory | Resolved or documented exception with regression proof | 01 | `npm audit --audit-level=moderate` no unreviewed high | PLANNED | `npm audit` output |
| PR-018 | §12 P1.4 | P1 | CI/security | Moderate PostCSS chain advisories | Safe resolution without Next downgrade | 01 | Audit clean or DEC-004 documented exception | PLANNED | `package-lock.json` |
...
| PR-044 | §12 P1.10 | P1 | Admin ops | Health marks DB healthy on execute alone | Probes distinguish configured/reachable/degraded | 02 | Health shows missing provider as not healthy | PLANNED | `/admin/health` + Phase 02 probes |
...
| PR-054 | §5.2; §12 P2 | P2 | Visitor signup | Signup/profile/invite not transactional | Compensating/idempotent signup flow | 04 | Concurrent signup + invite → one profile, no orphan | PLANNED | `signupAction` |
| PR-055 | §5.3; §12 P2 | P2 | Visitor OAuth | Beta invite not enforced in OAuth callback | OAuth obeys DEC-003 beta policy | 04 | OAuth without invite when required → blocked | PLANNED | `/auth/callback` |
| PR-056 | §12 P2; catalog idempotency | P2 | Billing | Callback dedupe check-then-insert | DB uniqueness + atomic idempotency | 03 | Parallel identical callbacks → one row | PLANNED | `mpesa_callbacks` |
...
| PR-088 | §12 P2; catalog support login | P1 | Support | Support login → "Unable to load account" | Support routed to `/admin` with correct role | 04 | Support OAuth/password → admin shell | PLANNED | `authService.ts` |
...
| PR-094 | catalog payment validation | P1 | Student billing | Amount/plan mismatch possible | Validate plan amount against platform settings | 03 | Wrong amount callback → rejected | PLANNED | payment validation |
| PR-095 | catalog phone validation | P1 | Student billing | Phone format not strictly validated | Merchant/phone normalization + rejection | 03 | Invalid phone → 400 before STK | PLANNED | stk-push route |
| PR-096 | catalog receipt uniqueness | P1 | Student billing | No uniqueness on provider receipt IDs | DB unique constraint on receipt/transaction IDs | 03 | Duplicate receipt → idempotent no-op | PLANNED | payments migration |
...
| PR-099 | catalog cron leakage | P2 | Security | Cron routes may leak error detail | Stable error codes; no stack to client | 04 | Cron failure → generic response | PLANNED | `/api/cron/weekly-reports` |
| PR-100 | catalog error codes | P2 | UX | Inconsistent client error codes | Stable coded errors for recovery UI | 04 | API contract tests for error shape | PLANNED | API schemas |
...
| PR-110 | §10 | P1 | Ops/deploy | No boot-time env validation | `env:check` on deploy + instrumentation | 02 | Server refuses traffic when prod misconfigured | PLANNED | instrumentation.ts |
| PR-111 | §7.2 | P1 | Notifications | `/api/celcom/webhook` unverified public | Verified secret/idempotency like payments | 03 | Forged Celcom webhook → no false delivery | PLANNED | celcom webhook |
| PR-113 | catalog polling | P1 | Student billing | No student payment status polling endpoint | Student-owned poll with auth + expiry UI | 03 | Pending payment → poll shows terminal state | PLANNED | payment status API |
| PR-114 | catalog duplicate pending | P1 | Student billing | Duplicate pending payments possible | Suppress duplicate pending per student/plan | 03 | Double STK click → one pending | PLANNED | stk-push |
| PR-115 | catalog expiry | P1 | Student billing | Pending payments never expire | Expiry job + UI recovery | 03 | Stale pending → expired recoverable | PLANNED | reconciliation job |
| PR-124 | catalog STK query | P0 | Student billing | No independent Daraja STK Query proof | Query before activation | 03 | Activation blocked until query success | PLANNED | mpesa client |
| PR-116 | §2; v2-tier-1 | P1 | Student Nex | Phase 2.5 voice batch verification unexecuted | Voice golden + E2E verification recorded | 01 | Tier 2.5 voice test suite PASS | PLANNED | `.planning/milestones/v2-tier-1/STATUS.md` |
| PR-117 | §11 | P1 | CI | CI not green on standalone typecheck | CI enforces same gates as deploy:check | 01 | GitHub workflow typecheck job green | PLANNED | `.github/workflows/ci.yml` |
| PR-118 | §11; Phase 01 | P1 | CI | `test:e2e:ci` not self-contained | Build, start server, readiness, teardown | 01 | `npm run test:e2e:ci` alone exits 0 | PLANNED | `package.json` |
| PR-119 | §11 | P1 | CI | `deploy:check` omits scope-check/audit | Unified release command | 01 | `npm run deploy:check` runs full matrix | PLANNED | `package.json` |
| PR-120 | STATUS | P1 | CI | `env:check` script missing | Script exists (stub ok in 01, strict in 02) | 01 | `npm run env:check` exists exit 0 in test env | PLANNED | `package.json` |
...
| PR-127 | catalog session revoke | P1 | Security | No session revocation on privilege change | Revoke sessions on role/plan change | 04 | Role demote → old JWT rejected | PLANNED | auth admin hooks |
...
| PR-142 | §3 | P2 | Security | Proxy matcher may omit newer student routes | Role matrix includes all 28 student pages | 04 | Matrix test every student `page.tsx` | PLANNED | `src/proxy.ts` |
